...
;(function ($) {
    ...
    LocationSetter.prototype = {
        ...
        initialize: function() {
            ...
            // 创建搜索框以及搜索结果容器
            container.html('<div class="search-box"><input type="text" class="input-search" /></div>'
                          +'<ul class="search-results list"></ul>');

            that.searchTextBox = $(".input-search", container);
            that.searchResultContainer = $(".search-results", container);

            that.searchTextBox.bind("keyup", function(e) { that.onKeyUp(e); });
        },
        /**
         * [onKeyUp description]
        ...
        getSuggestions: function(searchWord) {
            ...
            if (response && $.isArray(response.suggestions)) {
                ...
                that.currAjaxRequest = $.ajax(ajaxSettings);
            }
        },
        ...
        suggest: function() {
            ...
            $.each(results, function(i, suggestion) {
                itemsHtml += '<li class="search-item" data-id="' + suggestion[options.idField] + '" data-index="' + i + '">' + formatResult(suggestion[options.textField],  that.currSearchText)+ '</li>'
            });

            that.searchResultContainer.html(itemsHtml);

            $("li.search-item", that.searchResultContainer).bind("click", function(e) {
                var self = $(this);
        ...
        onSelect: function(index) {
            ...
            suggestion = options.suggestions[index];
